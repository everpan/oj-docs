<!-- 由 scripts/sync-docs.mjs 于 2026-09-18 从 `oj-module:docs/prd/framework-verification-playbook.md` 生成，请勿直接编辑；改源文件后运行 `npm run sync` -->
...
<p class="gen-note">generated: 2026-09-18 · 本页由脚本从 oj-module:docs/prd/framework-verification-playbook.md 同步生成，修改请改源文件后运行 npm run sync。</p>


# 框架端到端验证手册（可重复演练）

> **用途**：框架升级（`contract`/`runtime`/`shell`/`cli` 发新版，或 oj 升级）后，**从零**跑通「脚手架 → 依赖 → 契约 → 后端 → 前端 → 构建 → 运行」，一次确认整条链路没有回归。
> **配套**：[`framework-development-guide.md`](/frontend/framework-dev-guide/)（原理）、[`oj-fullstack-tutorial.md`](/frontend/fullstack-tutorial/)（跟做）。
...
> 完整根因与修复方案（可转上游）：[`oj-release-binary-defect-report.md`](/frontend/release-defect-report)。
...
[`oj-fullstack-tutorial.md`](/frontend/fullstack-tutorial/) 第 3–4 节），然后：
