...
hero:
  name: only-js
  text: 开发者手册
  tagline: 用 JS/TS 写业务，Rust 兜底后端能力；oj-module 提供 React + TS 前端框架，前后端一体
  actions:
    - theme: brand
      text: 从这里开始
...
features:
  - icon: 🧭
    title: 路由即目录
    details: 后端 src/user/profile/api.ts 自动变成 /v1/api/user/profile/，不用写路由表。
  - icon: 🧩
    title: 能力可插拔
  ...
  - icon: 🧱
    title: 前端框架（oj-module）
    details: React + TS 框架，runtime/cli 两个包发布到 npm；契约 → 前端 client 自动生成。
---
...
| 要写前端框架 / 业务模块 | [框架开发手册](./frontend/framework-dev-guide/index.md) → [全栈实战演练](./frontend/fullstack-tutorial/index.md) → [业务模块开发手册](./frontend/module-dev-guide/index.md) |
...
> 站点内容由 `scripts/sync-docs.mjs` 从后端仓库 `only-js`（`docs/`、`sample/`）与前端仓库
> `oj-module`（`docs/prd/`）同步生成，每页顶部标注了生成日期与源文件。
