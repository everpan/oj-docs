<!-- 由 scripts/sync-docs.mjs 于 2026-09-12 从 `only-js:docs/modules/README.md` 生成，请勿直接编辑；改源文件后运行 `npm run sync` -->
...
<p class="gen-note">generated: 2026-09-12 · 本页由脚本从 only-js:docs/modules/README.md 同步生成，修改请改源文件后运行 npm run sync。</p>
...
- [00 · 总览：分层、依赖与红线](/modules/00-overview) —— 第一次读仓库从这里开始
- [01 · 核心运行时 `src/bridge/`](/modules/01-core-bridge)
- [02 · 配置模型 `src/config.rs`](/modules/02-config)
- [03 · HTTP 服务 `server/`](/modules/03-server-http)
- [04 · CLI 与装配 `oj/`](/modules/04-oj-cli)
- [05 · FFI 契约与插件 `oj-plugin-ffi/` + `plugins/*`](/modules/05-ffi-and-plugins)
- [06 · 工具链 `tools/` · `benches/` · `tests/plugins/`](/modules/06-toolchain)
- [07 · 模块数据层 `oj/src/{manifest,schema,migrate,seed,checks}.rs`](/modules/07-data-layer)
- [08 · 测试分类与 `sample/test` vs `sample/tests` 处置](/modules/08-testing)

归档（**不描述当前实现**）：[`docs/archive/`](/appendix/history-index) —— 早期 CLI 预案、
rust-core-runtime 方案与评审、插件系统交接快照。

审查清单见 [`docs/review-2026-09-06.md`](/appendix/history-index)。
