<!-- 由 scripts/sync-docs.mjs 于 2026-09-12 从 `only-js:docs/websocket.md` 生成，请勿直接编辑；改源文件后运行 `npm run sync` -->
...
<p class="gen-note">generated: 2026-09-12 · 本页由脚本从 only-js:docs/websocket.md 同步生成，修改请改源文件后运行 npm run sync。</p>


# WebSocket 教学（ws.ts 生命周期钩子）

> 面向两类读者：**要写 WS handler 的业务开发者**（§1–§3、§7）与**要改 WS 实现的维护者**
> （§4–§6、§7 末）。JS 签名权威是 [devkit/api-manual.md](/reference/api-manual/)（§4 `ws.ts` 小节、
> §6 `ws`/`WebSocket`/`fetch` 行）；维护者模块地图是 [modules/03-server-http.md](/modules/03-server-http) §5。
...
（[mq-tasks.md](/reference/mq-tasks)）同构，但 **重连不用手写**：
