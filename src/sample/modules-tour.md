...
怎么跑通**。快速启动与部署命令见 [README.md](/sample/index)。
...
> WebSocket 专题教学见 [../docs/websocket.md](/reference/websocket)。
...
- `chat/ws.ts`：**帧内发布**聊天室——`connection()` 进房（订阅 "chat"，连上即完成，无需
  ...
  即广播给所有订阅者（含自己——自回声）。发布语义见 [../docs/websocket.md](/reference/websocket) §2。
...
完整 curl 接力（302 手动跟 Location）见 [README.md](/sample/index)「OIDC 演示」。
...
[../docs/migration.md](/reference/migration)。
